<?php
  ...
  if(isset($_POST["katakunci"])){
    ...
    $_SESSION['katakunci_blog'] = $katakunci_kategori;
  }
                      ...
                      <table class="table table-bordered">
                        ...
                        <tbody>
                          <?php
                            ...
                            $sql_g = "SELECT `b`.`id_blog`, `k`.`kategori_blog`,`b`.`judul`,`b`.`tanggal` 
                                      FROM `blog` `b`
                                      INNER JOIN `kategori_blog` `k` ON `b`.`id_kategori_blog` =`k`.`id_kategori_blog`";
                            if (!empty($katakunci_kategori)){
                              $sql_g .= " WHERE `b`.`judul` LIKE '%$katakunci_kategori%'";
                            }
                            $sql_g .= " ORDER BY `k`.`kategori_blog`, `b`.`judul` limit $posisi, $batas ";
                            $query_g = mysqli_query($koneksi,$sql_g);
                            $no = $posisi+1;
                            while($data_g = mysqli_fetch_row($query_g)){
                              $id_blog = $data_g[0];
                              $kategori_blog = $data_g[1];
                              $judul = $data_g[2];
                              $tanggal = $data_g[3];
                          ?>
                          <tr>
                            ...
                            <td align="center">
                              ...
                              'index.php?include=blog&aksi=hapus&data=<?php echo $id_blog; ?>¬if=hapusberhasil'"
